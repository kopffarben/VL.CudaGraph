using VL.Core;
using VL.Core.PublicAPI;

namespace Main;

/// <summary>
/// Responsible for invoking the logic defined in a <see cref="PureFunction"/>.
/// </summary>
[ProcessNode(FragmentSelection = FragmentSelection.Explicit)]
public class PureFunctionInvoker : IDisposable
{
    private NodeContext _nodeContext;
    private LFOInterface? _patchInlay;

    /// <summary>
    /// Initializes a new instance of the <see cref="PureFunctionInvoker"/> class.
    /// </summary>
    /// <param name="nodeContext">The node context provided by the system.</param>
    [Fragment(Order = 0)]
    public PureFunctionInvoker([Pin(Visibility = VL.Model.PinVisibility.Hidden)] NodeContext nodeContext)
    {
        _nodeContext = nodeContext;
    }

    /// <summary>
    /// Invokes the pure function logic.
    /// </summary>
    /// <param name="function">The pure function definition to invoke.</param>
    /// <param name="Period">The period for the LFO.</param>
    /// <param name="Pause">Whether to pause the LFO.</param>
    /// <param name="Reset">Whether to reset the LFO.</param>
    /// <param name="Result">The output result.</param>
    [Fragment(Order = 1)]
    public void Update(PureFunction? function, float Period, bool Pause, bool Reset, out float Result)
    {
        if (function == null)
        {
            Result = default;
            return;
        }

        if (function._patchInlayFactory != null && _patchInlay == null)
        {
            this._patchInlay = function._patchInlayFactory();
        }
        if (this._patchInlay != null)
        {
            this._patchInlay.Update(Period, Pause, Reset, out Result);
        }
        else
        {
            Result = default;
        }
    }

    #region IDisposable Members
    /// <inheritdoc />
    [Fragment(Order = 2)]
    public void Dispose()
    {
        if (_patchInlay is IDisposable disposable)
        {
            disposable.Dispose();
        }
    }
    #endregion IDisposable Members

}


/// <summary>
/// Represents a pure region handling LFO interface logic.
/// </summary>
[ProcessNode(FragmentSelection = FragmentSelection.Explicit, HasStateOutput = true)]
[Region(SupportedBorderControlPoints = ControlPointType.None)]
public class PureFunction : IRegion<LFOInterface>
{
    private NodeContext _nodeContext;
    internal Func<LFOInterface>? _patchInlayFactory;

    /// <summary>
    /// Initializes a new instance of the <see cref="PureFunction"/> class.
    /// </summary>
    /// <param name="nodeContext">The node context provided by the system.</param>
    [Fragment(Order = 0)]
    public PureFunction([Pin(Visibility = VL.Model.PinVisibility.Hidden)] NodeContext nodeContext)
    {
        _nodeContext = nodeContext;
    }

    /// <summary>
    /// Updates the region definition.
    /// </summary>
    [Fragment(Order = 1)]
    public void Update()
    {
        
    }

    #region IRegion<TestReginInterface> Members
    /// <inheritdoc />
    public void AcknowledgeInput(in InputDescription description, object? outerValue)
    {
        // no BorderControlPoints are supported, so this method will never be called
    }

    /// <inheritdoc />
    public void AcknowledgeOutput(in OutputDescription description, LFOInterface patchInlay, object? innerValue)
    {
        // no BorderControlPoints are supported, so this method will never be called
    }

    /// <inheritdoc />
    public void RetrieveInput(in InputDescription description, LFOInterface patchInlay, out object? innerValue)
    {
        // no BorderControlPoints are supported, so this method will never be called
        innerValue = null;
    }

    /// <inheritdoc />
    public void RetrieveOutput(in OutputDescription description, out object? outerValue)
    {
        // no BorderControlPoints are supported, so this method will never be called
        outerValue = null;
    }

    /// <summary>
    /// Sets the factory function used to create the patch inlay instance.
    /// </summary>
    /// <param name="patchInlayFactory">The factory function.</param>
    public void SetPatchInlayFactory(Func<LFOInterface> patchInlayFactory)
    {
        this._patchInlayFactory = patchInlayFactory;
    }
    #endregion IRegion<TestReginInterface> Members

}