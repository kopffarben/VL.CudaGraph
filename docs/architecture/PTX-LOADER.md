...
- **CUDA C/C++** (nvcc ‑‑ptx) — full control, industry standard
- **Numba** (Python) — lightweight Python → PTX path
- **Hand-written PTX** — maximum control, niche use

```
Build-Time (any toolchain)          Runtime (C#)
                                    
    Kernel Source                   ┌──────────────┐
    (.py / .cu / .ptx)              │  PTX Loader  │
           │                        │              │
           ▼                        │  - Parse     │
    Compile to PTX                  │  - Validate  │
           │                        │  - Cache     │
           ▼                        │              │
    ┌──────────────────┐           │              │
    │ kernel_name.ptx  │──────────▶│              │
...
## PTX Authoring Examples

The following examples show different toolchains. All produce the same output:
a `.ptx` file and an accompanying `.json` metadata file.

### Example 1: Triton (Python)
...
def compile_kernel(kernel_fn, signature, output_path):
    compiled = compile(
        kernel_fn,
        signature=signature,
        # Target compute capability (minimum: SM 7.5 / Turing)
        cc=(7, 5),  # SM 7.5 — our minimum, runs on RTX 20xx+
    )
...
extern "C" __global__
void vector_add_f32(const float* A, const float* B, float* C, unsigned int n)
{
...
```bash
nvcc --ptx -arch=sm_75 -o kernels/vector_add.ptx kernels/vector_add.cu
```

The JSON metadata file is the same format as above — create `kernels/vector_add.json` manually or with a generation script. The PTX Loader does not care which compiler produced the `.ptx`.

### Example 3: Numba (Python)
...
# Compile to PTX
ptx, resty = cuda.compile_ptx(vector_add_kernel, (float32[:], float32[:], float32[:], int32))
with open("kernels/vector_add.ptx", "w") as f:
    f.write(ptx)
```

Same JSON metadata format applies.
...
```ptx
//
// Generated by CUDA compiler (Triton, nvcc, etc.)
//
.version 8.4
.target sm_75
.address_size 64
...
5. **Target SM 7.5**: Our minimum — compiles will JIT-upscale to the user's actual GPU
